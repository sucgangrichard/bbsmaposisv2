            ...
            <div class="card-body">
              ...
              <form action="" method="post" name="">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control select2bs4 select2 warning" id="select6" style="width: 100%;" onchange="disableSelectedOption(this)">
...
function disableSelectedOption(selectElement) {
  ...
  var selectedText = options[selectElement.selectedIndex].text;
  ...
  options[selectElement.selectedIndex].style.backgroundColor = "#d3d3d3"; // Set background color for disabled option

  // Check if the selected item matches any table data
  for (var i = 1; i < rows.length; i++) { // Start from 1 to skip the header row
    var cells = rows[i].getElementsByTagName('td');
    if (cells[1].innerText === selectedText) { // Assuming the product name is in the second column
      matchFound = true;
      originalQty = cells[0].querySelector('.qty').value; // Assuming the quantity is in the first column
      break;
    }
  }

  if (matchFound) {
    alert("Warning: The product '" + selectedText + "' is already in the table.");
    // Reset the quantity back to the original one
    for (var i = 1; i < rows.length; i++) { // Start from 1 to skip the header row
      var cells = rows[i].getElementsByTagName('td');
      if (cells[1].innerText === selectedText) { // Assuming the product name is in the second column
        cells[0].querySelector('.qty').value = originalQty; // Reset the quantity
        break;
      }
    }
    selectElement.selectedIndex = 0; // Reset the select element to its default state
  } else {
    // Add the new row if no match is found
    addrow(selectElement.value, selectedText, 1, 0); // Assuming default values for available and price
  }
}
</script>
